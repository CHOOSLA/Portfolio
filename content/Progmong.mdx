...
challenges:
  - title: "몰입형 게임 경험과 모바일 편의성을 동시에 잡는 Dual-Layout 전략"
    ...
    description: "일반적인 반응형 웹(Fluid Layout) 방식으로는 아트워크 중심의 고정 비율 게임 UI(데스크탑)와 세로 스크롤 중심의 모바일 UI를 동시에 만족시키기 불가능했습니다. 데스크탑에서는 게임의 몰입감을 위해 레이아웃이 깨지지 않아야 했고, 모바일에서는 작은 화면에서도 조작이 편리해야 했습니다."
    technicalDetails:
